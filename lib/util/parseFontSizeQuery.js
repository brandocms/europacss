...
function parseFontSizeQuery(node, config, fontSizeQuery, breakpoint) {
    ...
    if (hasSlash) {
        ...
        for(let i = slashCount; i >= 1; i--){
            ...
            resolvedFontsize = _lodash.default.get(config, themePath.concat(potentialKey));
            // If not found, try path traversal (convert slashes to dots)
            if (!resolvedFontsize) {
                const pathParts = potentialKey.split('/');
                resolvedFontsize = _lodash.default.get(config, themePath.concat(pathParts));
            }
            if (resolvedFontsize) {
        ...
        if (!foundKey) {
            ...
            resolvedFontsize = (0, _resolveConfigKey.default)(config, themePath, fontSizeQuery);
            if (!resolvedFontsize) {
    ...
    if (!_lodash.default.isString(resolvedFontsize)) {
        ...
        if (!_lodash.default.has(resolvedFontsize, breakpoint)) {
            throw node.error(`FONTSIZE: No breakpoint \`${breakpoint}\` found in theme.typography.sizes.${fontSizeQuery}`, {
                name: breakpoint
            });
        }
    }
