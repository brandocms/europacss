...
const _resolveConfigKey = /*#__PURE__*/ _interop_require_default(require("./resolveConfigKey"));
 ...
 */ function processVerticalRhythm(size, config, bp, node) {
    ...
    const obj = (0, _resolveConfigKey.default)(config, [], key);
    // does it exist?
    if (!obj) {
 ...
 */ function processContainerSize(size, config, bp, node) {
    ...
    if (rawPaddingValue && typeof rawPaddingValue === 'string' && rawPaddingValue.startsWith('*')) {
        ...
        if (parts.length > 1) {
            ...
            const [val, unit] = (0, _splitUnit.default)(minPadding);
            if (unit === DPX_UNIT) {
        ...
        const [val, unit] = (0, _splitUnit.default)(rawPaddingValue);
        ...
        case CONTAINER_HALF:
            {
                ...
                const [paddingVal, paddingUnit] = (0, _splitUnit.default)(paddingValue);
 ...
 */ function processCalcExpression(size, config, bp, node) {
    ...
    matches.forEach((m)=>{
        ...
        const needsParens = !parsedMatch.startsWith('(') && !parsedMatch.startsWith('calc(') && (parsedMatch.includes('+') || parsedMatch.includes('-') || parsedMatch.includes('*') || parsedMatch.includes('/'));
    ...
    if (size.indexOf(DPX_UNIT) !== -1) {
        ...
        const calcContent = ((_size_match = size.match(/calc\((.*)\)/)) === null || _size_match === void 0 ? void 0 : _size_match[1]) || size;
 ...
 */ function processComplexFraction(size, config, bp, node) {
    ...
    let [gutterValue, gutterUnit] = (0, _splitUnit.default)(processedGutterSize);
 ...
 */ function processVwValue(size, config, bp, node, applyZoom = false) {
    ...
    if (config.hasOwnProperty('setMaxForVw') && config.setMaxForVw === true && (0, _isLargestBreakpoint.default)(config, bp)) {
        const maxSize = (0, _getLargestContainer.default)(config);
        const [valMax, unitMax] = (0, _splitUnit.default)(maxSize);
        if (unitMax === '%') {
            throw node.error(`SPACING: When setMaxForVw is true, the container max cannot be % based.`);
...
function parseSize(node, config, size, bp) {
    ...
    if (size && size.startsWith('-') && _lodash.default.has(config.theme.spacing, size.substring(1))) {
        multiplier = -1;
        size = size.substring(1);
    }
    // Check if size is a named spacing value in config (flat key or nested path)
    let sizeMap;
    const resolvedSpacing = (0, _resolveConfigKey.default)(config, [
        'theme',
        'spacing'
    ], size);
    if (resolvedSpacing !== undefined) {
        sizeMap = (0, _replaceWildcards.default)(resolvedSpacing, config);
        size = sizeMap[bp];
