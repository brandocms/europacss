 ...
 */ function getByPath(theme, path) {
    const segments = (0, _toPath.default)(path);
    let current = theme;
...
function resolveTokenReferences(theme) {
            ...
            if (TOKEN_REF_RE.test(val)) {
                ...
                return val.replace(TOKEN_REF_RE, (_, refPath)=>{
                    ...
                    if (typeof refValue !== 'string' && typeof refValue !== 'number') {
                        throw new Error(`Token reference error: Cannot interpolate {${refPath}} into string at "${currentPath}" — referenced value is a ${typeof refValue}, not a string or number`);
                    }
                    return refValue;
                });
            }
   ...
   */ function resolveRef(refPath, fromPath) {
        ...
        if (resolving.has(refPath)) {
            throw new Error(`Token reference error: Circular reference detected — "${fromPath}" references "{${refPath}}" which is already being resolved`);
        }
        const rawValue = getByPath(theme, refPath);
        if (rawValue === undefined) {
            throw new Error(`Token reference error: "{${refPath}}" referenced from "${fromPath}" does not exist in the theme`);
        }
