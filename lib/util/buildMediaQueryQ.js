...
function buildMediaQueryQ({ breakpoints, breakpointCollections }, q) {
    ...
    const sortedQueryStrings = _lodash.default.sortBy(optimizedQueryStrings, [
        (query)=>parseInt((query.min || '0').replace('px', ''), 10)
    ]);
    return (0, _lodash.default)(sortedQueryStrings).map((screen)=>{
        // Skip min constraint if it's 0
        const filterZeroMin = _lodash.default.omitBy(screen, (value, feature)=>feature === 'min' && (value === '0' || value === '0px'));
        return (0, _lodash.default)(filterZeroMin).map((value, feature)=>{
