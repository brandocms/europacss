 ...
 */ function resolveBasePropertyValue(value, prop, theme, atRule) {
    ...
    if (_lodash.default.isFunction(value)) {
        ...
        if (resolved === undefined || resolved === null) {
            throw atRule.error(`FONTSIZE: Function in __base__ for '${prop}' returned ${resolved}. ` + `Check that the configuration path exists in your theme.`);
        }
...
function processRule(atRule, config) {
    ...
    let resolvedConfig = (0, _resolveConfigKey.default)(config, themePath, fontSizeQuery);
    ...
    const isLiteralValue = _lodash.default.isString(fontSizeQuery) && !(0, _resolveConfigKey.default)(config, themePath, fontSizeName);
    if (responsiveParent && isLiteralValue) {
        // Literal value under @responsive — output inline, no media query needed
        const firstBp = (0, _extractBreakpointKeys.default)({
            breakpoints,
            breakpointCollections
        }, bpQuery)[0];
        let parsedFontSizeQuery = (0, _parseFontSizeQuery.default)(clonedRule, config, fontSizeQuery, firstBp);
