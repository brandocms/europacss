 ...
 */ function resolveNegateArg(node, config, expr, bp) {
    if (expr.includes('+')) {
        ...
        const resolved = parts.map((p)=>{
            let val = (0, _parseSize.default)(node, config, p, bp);
    ...
    if (dashIdx > 0) {
        ...
        let leftVal = (0, _parseSize.default)(node, config, left, bp);
        let rightVal = (0, _parseSize.default)(node, config, right, bp);
    ...
    let val = (0, _parseSize.default)(node, config, expr, bp);
    if (val.startsWith('calc(') && val.endsWith(')')) {
        val = val.substring(5, val.length - 1);
    }
    return val;
 ...
 */ function buildNegateValue(node, config, currentGap, desiredGap, bp) {
    let resolvedCurrent = (0, _parseSize.default)(node, config, currentGap, bp);
 ...
 */ function processNegateRule(atRule, config, flagAsImportant, clonedRule, currentGap, desiredGap, bpQuery) {
    ...
    if (responsiveParent) {
        if (bpQuery) {
            throw clonedRule.error(`SPACING: @space negate() cannot be nested under @responsive and have a breakpoint query.`, {
                name: bpQuery
            });
        }
    ...
    if (bpQuery) {
        const affectedBreakpoints = parsedBreakpoints || (0, _extractBreakpointKeys.default)({
            breakpoints,
            breakpointCollections
        }, bpQuery);
        _lodash.default.each(affectedBreakpoints, (bp)=>{
            const value = buildNegateValue(clonedRule, config, currentGap, desiredGap, bp);
            const decl = _postcss.default.decl({
                prop: 'margin-top',
                value,
                important: flagAsImportant
            });
...
function processRule(atRule, config, flagAsImportant) {
    ...
    if (prop === 'breakout') {
        const sizeDecls = (0, _buildDecl.default)('breakout', null, flagAsImportant);
        if (bpQuery) {
            const mediaRule = clonedRule.clone({
                name: 'media',
                params: (0, _buildMediaQueryQ.default)({
